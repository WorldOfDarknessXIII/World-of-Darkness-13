...
/atom/movable/screen/alert/untorpor/Click()
	...
	if (!ishuman(usr))
		return
	var/mob/living/carbon/human/vampire = usr
	if (vampire.stat >= DEAD)
		to_chat(owner, span_warning("You can't wake up from your torpor due to being dead!"))
		return

	var/datum/splat/vampire/kindred/vampirism = is_kindred(vampire)
	var/datum/splat/hungry_dead/kuei_jin/kuei_jin = is_kuei_jin(vampire)
	if (vampirism)
		if (COOLDOWN_FINISHED(vampirism, torpor_timer) && (vampire.bloodpool > 0))
			vampire.untorpor()
			INVOKE_ASYNC(vampire, TYPE_PROC_REF(/mob, clear_alert), "succumb")
		else
			...
			if (vampire.bloodpool > 0)
				to_chat(usr, "<span class='purple'><i>You will be able to awaken in <b>[DisplayTimeText(COOLDOWN_TIMELEFT(vampirism, torpor_timer))]</b>.</i></span>")
		...
		if (COOLDOWN_FINISHED(kuei_jin, torpor_timer) && (vampire.yang_chi > 0 || vampire.yin_chi > 0))
			vampire.untorpor()
			INVOKE_ASYNC(vampire, TYPE_PROC_REF(/mob, clear_alert), "succumb")
		else
			...
			if (vampire.yang_chi > 0 || vampire.yin_chi > 0)
				to_chat(usr, "<span class='purple'><i>You will be able to awaken in <b>[DisplayTimeText(COOLDOWN_TIMELEFT(kuei_jin, torpor_timer))]</b>.</i></span>")

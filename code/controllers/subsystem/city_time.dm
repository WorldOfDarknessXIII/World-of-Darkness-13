...
/datum/controller/subsystem/city_time/fire()
	...
	if(hour == 6 && minutes == 0)
		...
		for(var/mob/living/carbon/human/human in GLOB.human_list)
			var/area/vtm/in_area = get_area(human)
			if (!in_area)
				continue
			if (!istype(in_area, /area/vtm))
				continue
			if (in_area.upper)
				//these effects suck and should be made better
				if (HAS_TRAIT(human, TRAIT_BURNS_IN_SUNLIGHT))
					human.death()
				if (HAS_TRAIT(human, TRAIT_ROTS_IN_SUNLIGHT))
					human.death()

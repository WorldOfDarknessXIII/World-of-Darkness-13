...
/proc/healthscan(mob/user, mob/living/M, mode = SCANNER_VERBOSE, advanced = FALSE)
	...
	if(iskindred(M) || (iscathayan(M) && humhum.mind.dharma?.animated != "Yang"))
	...
	if(ishuman(M))
		...
		if (H.internal_organs && H.internal_organs.len)
			...
			for(var/obj/item/organ/organ in H.internal_organs)
				var/status = ""
				if (iskindred(H) || (iscathayan(H) && H.mind.dharma?.animated != "Yang")) status = "<font color='#E42426'>Non-Functional</font>"

	...
	switch(objective)
		...
		if(3)
			...
			for(var/mob/living/carbon/human/H in GLOB.player_list)
				if(H.stat != DEAD && H.true_real_name != owner.current.true_real_name &~ IS_SABBAT(H))
					ambitious += H
		...
		if(4)
			if(needs_faction)
				...
				while(owner.current.vampire_faction == null && retries > 0)
					...
					retries -= 1
			var/list/available_factions = list(/datum/faction/camarilla, /datum/faction/anarchs, /datum/faction/sabbat)
			if(owner.current.vampire_faction == null || owner.current.vampire_faction == /datum/faction/nosferatu)
				no_faction = TRUE
			if(no_faction)

...
obj/effect/fire/baali
	color = "#2dff00"

/obj/effect/fire/baali/proc/handle_automated_spread()
		...
		for(var/turf/open/floor/A in range(1, src))
			...
			if(allowed_to_spread)
				if(A != loc && A.burn_material)
					var/obj/effect/fire/baali/F = locate() in A
					if(!F && prob(A.spread_chance))
						playsound(get_turf(A), 'code/modules/wod13/sounds/spread.ogg', 80, TRUE)
						var/obj/effect/fire/baali/R = new(A)
						R.color = color
	else
		qdel(src)

/obj/effect/fire/baali/Crossed(atom/movable/AM, oldloc)
	. = ..()
	if(isliving(AM))
		var/mob/living/L = AM
		L.fire_stacks += 5
...
/obj/effect/fire/baali/Destroy()
	. = ..()
	set_light(0)
	GLOB.fires_list -= src

/obj/effect/fire/baali/Initialize()
	. = ..()
